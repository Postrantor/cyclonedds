...
#define DDS_RETCODE_OUT_OF_RESOURCES \
  (-5)                               /**< When an operation fails because of a lack of resources */
#define DDS_RETCODE_NOT_ENABLED (-6) /**< When a configurable feature is not enabled */
#define DDS_RETCODE_IMMUTABLE_POLICY \
  (-7) /**< When an attempt is made to modify an immutable policy */
#define DDS_RETCODE_INCONSISTENT_POLICY (-8) /**< When a policy is used with inconsistent values */
#define DDS_RETCODE_ALREADY_DELETED \
  (-9)                            /**< When an attempt is made to delete something more than once */
...
#define DDS_RETCODE_NOT_ALLOWED_BY_SECURITY \
  (-13) /**< When credentials are not enough to use the function */

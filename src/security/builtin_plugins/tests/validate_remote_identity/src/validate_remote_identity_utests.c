...
static const char * SUBJECT_NAME_IDENTITY_CERT_2 =
  "CN=CHAM-574_1 client,O=Some Company,ST=Some-State,C=NL";
static const char * SUBJECT_NAME_IDENTITY_CA_2 =
  "CN=CHAM-574_1 authority,O=Some Company,ST=Some-State,C=NL";
...
static void initialize_identity_token_w_sn(
  DDS_Security_IdentityToken * token, const char * certSubjName, const char * certAlgo,
  const char * caSubjName, const char * caAlgo)
{
  memset(token, 0, sizeof(*token));
...
static int create_local_identity(void)
{
  int res = 0;
...
static int check_auth_request_token(DDS_Security_AuthRequestMessageToken * token, int notNil)
{
  if (notNil) {
    if (
      !token->class_id ||
    ...
    if (token->binary_properties._length != 1 || token->binary_properties._buffer == NULL) {
      printf("AuthRequestMessageToken has binary_properties\n");
      return 0;
    }

    if (
      !token->binary_properties._buffer[0].name ||
      (strcmp(token->binary_properties._buffer[0].name, DDS_AUTHTOKEN_PROP_FUTURE_CHALLENGE) !=
       0)) {
    ...
    if (
      token->binary_properties._buffer[0].value._length != 32 ||
    ...
    if (
      (strlen(token->class_id) != 0) || (token->properties._length != 0) ||
      (token->properties._maximum != 0) || (token->binary_properties._buffer != NULL) ||
      (token->binary_properties._length != 0) || (token->binary_properties._maximum != 0) ||
...
static void set_remote_participant_guid(DDS_Security_GUID_t * guid, int higher)
{
  int i;
...
CU_Test(ddssec_builtin_validate_remote_identity, happy_day_nil_auth_req)
{
  ...
  result = auth->validate_remote_identity(
    auth, &remote_identity_handle, &local_auth_request_token, NULL, local_identity_handle,
    &remote_identity_token, &remote_participant_guid, &exception);
  ...
  if (
    ...
    DDS_Security_boolean success =
      auth->return_identity_handle(auth, remote_identity_handle, &exception);
...
CU_Test(ddssec_builtin_validate_remote_identity, invalid_parameters)
{
  ...
  result = auth->validate_remote_identity(
    auth, NULL, &local_auth_request_token, &remote_auth_request_token, local_identity_handle,
    &remote_identity_token, &remote_participant_guid, &exception);
  if (result != DDS_SECURITY_VALIDATION_OK) {
    printf(
      "validate_local_identity_failed: %s\n",
  ...
  result = auth->validate_remote_identity(
    auth, &remote_identity_handle, NULL, &remote_auth_request_token, local_identity_handle,
    &remote_identity_token, &remote_participant_guid, &exception);
  if (result != DDS_SECURITY_VALIDATION_OK) {
    printf(
      "validate_local_identity_failed: %s\n",
...
CU_Test(ddssec_builtin_validate_remote_identity, unknown_local_identity)
{
  ...
  result = auth->validate_remote_identity(
    auth, &remote_identity_handle, &local_auth_request_token, &remote_auth_request_token,
    unknown_identity_handle, &remote_identity_token, &remote_participant_guid, &exception);
...
CU_Test(ddssec_builtin_validate_remote_identity, invalid_auth_req_token)
{
  ...
  ddsrt_free(remote_auth_request_token.binary_properties._buffer[0].name);
  remote_auth_request_token.binary_properties._buffer[0].name =
    ddsrt_strdup(DDS_AUTHTOKEN_PROP_FUTURE_CHALLENGE);
...
CU_Test(ddssec_builtin_validate_remote_identity, already_validated_same_token)
{
  ...
  result = auth->validate_remote_identity(
    auth, &remote_identity_handle2, &local_auth_request_token, &remote_auth_request_token,
    local_identity_handle, &remote_identity_token, &remote_participant_guid, &exception);
...
CU_Test(ddssec_builtin_validate_remote_identity, already_validated_different_token)
{
  ...
  initialize_identity_token_w_sn(
    &remote_identity_token2, SUBJECT_NAME_IDENTITY_CERT_2, RSA_2048_ALGORITHM_NAME,
    SUBJECT_NAME_IDENTITY_CA_2, RSA_2048_ALGORITHM_NAME);

  result = auth->validate_remote_identity(
    auth, &remote_identity_handle2, &local_auth_request_token, &remote_auth_request_token,
    local_identity_handle, &remote_identity_token2, &remote_participant_guid, &exception);

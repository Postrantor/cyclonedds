...
static const char * SUBJECT_NAME_PERMISSIONS_CA =
  "/C=NL/ST=OV/L=Locality Name/OU=Organizational Unit Name/O=Example Signer CA/CN=Example "
  "CA/emailAddress=authority@identitycaltd.org";
...
static dds_time_t get_certificate_expiry(
  /*_In_*/ X509 * cert)
{
  ...
  if (ans1 != NULL) {
    ...
    if (ASN1_TIME_diff(&days, &seconds, NULL, ans1) != 0) {
      ...
      dds_duration_t delta =
        DDS_SECS(((dds_duration_t)seconds + ((dds_duration_t)days * secs_per_day)));
...
static DDS_Security_boolean create_certificate_from_csr(
  const char * csr, long valid_secs, const char * outfile, dds_time_t * expiry)
{
...
static void fill_participant_qos(
  DDS_Security_Qos * qos, long validity_duration, const char * governance_filename,
  dds_time_t * identity_expiry)
{
  ...
  create_certificate_from_csr(
    alice_csr, validity_duration, ALICE_IDENTITY_CERT_FILE, identity_expiry);
...
static int fill_peer_credential_token(DDS_Security_AuthenticatedPeerCredentialToken * token)
{
  int result = 1;
  ...
  DDS_Security_IdentityToken * token, const char * certAlgo, const char * caAlgo)
{
  ...
  token->properties._buffer[0].value = ddsrt_strdup(
    "/C=NL/ST=OV/L=Locality Name/OU=Organizational Unit Name/O=Example Organization/CN=Bob "
    "Example/emailAddress=bob@exampleltd.com");
  ...
  token->properties._buffer[2].value = ddsrt_strdup(
    "/C=NL/ST=OV/L=Locality Name/OU=Organizational Unit Name/O=Example Signer CA/CN=Example "
    "CA/emailAddress=authority@identitycaltd.org");
...
static DDS_Security_long validate_local_identity_and_permissions(
  uint32_t identity_expiry_duration, dds_time_t * identity_expiry)
{
  ...
  fill_participant_qos(
    &participant_qos, (long)identity_expiry_duration, "Example_Governance.p7s", identity_expiry);
  ...
  if (result != DDS_SECURITY_VALIDATION_OK) {
    ...
    printf(
      "validate_local_identity_failed: (%d) %s\n", (int)exception.code,
  ...
  if (res == 0) {
    local_permissions_handle = access_control->validate_local_permissions(
      access_control, auth, local_identity_handle, 0, &participant_qos, &exception);

    if (local_permissions_handle == DDS_SECURITY_HANDLE_NIL) {
      printf(
        "validate_local_permissions_failed: (%d) %s\n", (int)exception.code,
        exception.message ? exception.message : "Error message missing");
...
static void clear_local_identity_and_permissions(void)
{
  ...
  if (local_permissions_handle != DDS_SECURITY_HANDLE_NIL) {
    success = access_control->return_permissions_handle(
      access_control, local_permissions_handle, &exception);
    if (!success) {
      printf(
        "return_permission_handle failed: %s\n",
        exception.message ? exception.message : "Error message missing");
    }
    reset_exception(&exception);
  }
...
static int set_path_to_etc_dir(void)
{
  int res = 0;
  size_t len;
...
static char * get_openssl_error_message_for_test(void)
{
...
static const BIGNUM * dh_get_public_key(
  /*_In_ */ DH * dhkey)
{
...
CU_Init(ddssec_builtin_listeners_auth)
{
  ...
  plugins = load_plugins(
    &access_control /* Access Control */, &auth /* Authentication */, NULL /* Cryptograpy    */,
...
static DDS_Security_ValidationResult_t create_asymmetrical_signature_for_test(
  EVP_PKEY * pkey, void * data, size_t dataLen, unsigned char ** signature, size_t * signatureLen,
  DDS_Security_SecurityException * ex)
{
  ...
  if (!(mdctx = EVP_MD_CTX_create())) {
    ...
    DDS_Security_Exception_set(
      ex, "Authentication", DDS_SECURITY_ERR_UNDEFINED_CODE, (int)result,
      "Failed to create signing context: %s", msg);
    ddsrt_free(msg);
    goto err_create_ctx;
  }
  ...
  if (EVP_DigestSignInit(mdctx, &kctx, EVP_sha256(), NULL, pkey) != 1) {
    ...
    DDS_Security_Exception_set(
      ex, "Authentication", DDS_SECURITY_ERR_UNDEFINED_CODE, (int)result,
      "Failed to initialize signing context: %s", msg);
    ddsrt_free(msg);
    goto err_sign;
  }
  ...
  if (EVP_DigestSignUpdate(mdctx, data, dataLen) != 1) {
    ...
    DDS_Security_Exception_set(
      ex, "Authentication", DDS_SECURITY_ERR_UNDEFINED_CODE, (int)result,
      "Failed to update signing context: %s", msg);
    ddsrt_free(msg);
    goto err_sign;
  }
  ...
  if (EVP_DigestSignFinal(mdctx, NULL, signatureLen) != 1) {
    ...
    DDS_Security_Exception_set(
      ex, "Authentication", DDS_SECURITY_ERR_UNDEFINED_CODE, (int)result,
      "Failed to finalize signing context: %s", msg);
    ddsrt_free(msg);
    goto err_sign;
  }
...
static DDS_Security_ValidationResult_t create_signature_for_test(
  EVP_PKEY * pkey, const DDS_Security_BinaryProperty_t ** binary_properties,
  const uint32_t binary_properties_length, unsigned char ** signature, size_t * signatureLen,
  DDS_Security_SecurityException * ex)
{
  DDS_Security_ValidationResult_t result;
  DDS_Security_Serializer serializer;
  ...
  DDS_Security_Serialize_BinaryPropertyArray(
    serializer, binary_properties, binary_properties_length);
  DDS_Security_Serializer_buffer(serializer, &buffer, &size);
...
static int validate_remote_identity(const char * remote_id_certificate)
{
  int res = 0;
  DDS_Security_ValidationResult_t result;
  ...
  result = auth->validate_remote_identity(
    auth, &remote_identity_handle, &g_local_auth_request_token, NULL, local_identity_handle,
  ...
  switch (step) {
    case HANDSHAKE_REQUEST:
      ...
      /* Calculate the hash_c1 */
      {
        ...
        set_binary_property_value(
          hash_c1, DDS_AUTHTOKEN_PROP_HASH_C1, hash1_sent_in_request_arr,
          sizeof(hash1_sent_in_request_arr));
      }
    ...
    case HANDSHAKE_REPLY:
      ...
      /* Calculate the hash_c2 */
      {
        ...
        set_binary_property_value(
          hash_c2, DDS_AUTHTOKEN_PROP_HASH_C2, hash2_sent_in_reply_arr,
          sizeof(hash2_sent_in_reply_arr));
      }
  ...
  const dds_security_authentication * plugin, const DDS_Security_IdentityHandle handle)
{
  ...
  printf(
    "Listener called for handle: %lld  Local:%lld Remote:%lld\n", (long long)handle,
    (long long)local_identity_handle, (long long)remote_identity_handle2);
...
CU_Test(ddssec_builtin_listeners_auth, local_remote_set_before_validation)
{
  ...
  access_control->set_listener(access_control, &access_control_listener, &exception);
  ...
  fill_handshake_message_token(
    ...
    hash1_sent_in_request, NULL, HANDSHAKE_REPLY);
  ...
  result = access_control->validate_remote_permissions(
    access_control, auth, local_identity_handle, remote_identity_handle, &permissions_token,
    &credential_token, &exception);

  if (result == 0) {
    printf(
      "validate_remote_permissions_failed: %s\n",
      exception.message ? exception.message : "Error message missing");

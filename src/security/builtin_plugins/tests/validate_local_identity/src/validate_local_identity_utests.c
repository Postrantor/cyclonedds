...
CU_Init(ddssec_builtin_validate_local_identity)
{
  ...
  plugins = load_plugins(
    NULL /* Access Control */, &auth /* Authentication */, NULL /* Cryptograpy    */, NULL);
  return plugins ? 0 : -1;
}
...
static void print_guid(const char * msg, DDS_Security_GUID_t * guid)
{
  uint32_t i, j;
...
static void fill_participant_qos(
  DDS_Security_Qos * participant_qos, bool is_file_certificate, const char * certificate,
  bool is_file_ca, const char * ca, bool is_file_private_key, const char * private_key_data,
  const char * password, const char * trusted_ca_dir, bool is_file_crl, const char * crl_data)
{
...
#ifdef WIN32
    snprintf(
      identity_cert_path, 1024, "file:%s\\validate_local_identity\\etc\\%s", CONFIG_ENV_TESTS_DIR,
      certificate);
#else
    snprintf(
      identity_cert_path, 1024, "file:%s/validate_local_identity/etc/%s", CONFIG_ENV_TESTS_DIR,
...
#ifdef WIN32
    snprintf(
      identity_CA_path, 1024, "file:%s\\validate_local_identity\\etc\\%s", CONFIG_ENV_TESTS_DIR,
      ca);
#else
    snprintf(
      identity_CA_path, 1024, "file:%s/validate_local_identity/etc/%s", CONFIG_ENV_TESTS_DIR, ca);
#endif
...
#ifdef WIN32
    snprintf(
      private_key_path, 1024, "file:%s\\validate_local_identity\\etc\\%s", CONFIG_ENV_TESTS_DIR,
      private_key_data);
#else
    snprintf(
      private_key_path, 1024, "file:%s/validate_local_identity/etc/%s", CONFIG_ENV_TESTS_DIR,
...
#ifdef WIN32
      snprintf(
        crl_path, 1024, "file:%s\\validate_local_identity\\etc\\%s", CONFIG_ENV_TESTS_DIR,
        crl_data);
#else
      snprintf(
        crl_path, 1024, "file:%s/validate_local_identity/etc/%s", CONFIG_ENV_TESTS_DIR, crl_data);
#endif
...
#ifdef WIN32
    snprintf(
      trusted_ca_dir_path, 1024, "%s\\validate_local_identity\\etc\\%s", CONFIG_ENV_TESTS_DIR,
      trusted_ca_dir);
#else
    snprintf(
      trusted_ca_dir_path, 1024, "%s/validate_local_identity/etc/%s", CONFIG_ENV_TESTS_DIR,
...
CU_Test(ddssec_builtin_validate_local_identity, happy_day)
{
  ...
  DDS_Security_GuidPrefix_t prefix = {0xf0, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5,
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, false, identity_ca, false, private_key, NULL,
    NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
  ...
  fill_participant_qos(
    &participant_qos, true, identity_certificate_filename, true, identity_ca_filename, true,
    private_key_filename, NULL, NULL, false, NULL);
...
CU_Test(ddssec_builtin_validate_local_identity, invalid_certificate)
{
  ...
  fill_participant_qos(
    &participant_qos, false, invalid_identity_certificate, false, identity_ca, false, private_key,
  ...
  fill_participant_qos(
    &participant_qos, true, invalid_identity_certificate_filename, false, identity_ca, false,
    private_key, NULL, NULL, false, NULL);
...
CU_Test(ddssec_builtin_validate_local_identity, invalid_root)
{
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, false, invalid_identity_ca, false, private_key,
    NULL, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, true, invalid_identity_ca_filename, false,
    private_key, NULL, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
...
CU_Test(ddssec_builtin_validate_local_identity, invalid_chain)
{
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, false, unrelated_identity_ca, false, private_key,
    NULL, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, true, unrelated_identity_ca_filename, false,
    private_key, NULL, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
...
CU_Test(ddssec_builtin_validate_local_identity, certificate_key_too_small)
{
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate_1024key, false, identity_ca, false, private_key,
    NULL, NULL, false, NULL);
...
CU_Test(ddssec_builtin_validate_local_identity, invalid_private_key)
{
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, false, identity_ca, false, invalid_private_key,
    NULL, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, false, identity_ca, true,
    invalid_private_key_filename, NULL, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
    &candidate_participant_guid, &exception);
...
CU_Test(ddssec_builtin_validate_local_identity, private_key_too_small)
{
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, false, identity_ca, false, private_key_1024,
    NULL, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
...
CU_Test(ddssec_builtin_validate_local_identity, missing_certificate_property)
{
  ...
  participant_qos.property.value._buffer[0].name =
    ddsrt_strdup(DDS_SEC_PROP_PREFIX
                 "auth.identity_cert");  // deliberately *not* DDS_SEC_PROP_AUTH_IDENTITY_CERT
  ...
  CU_ASSERT(
    strcmp(
      exception.message,
      "validate_local_identity: missing property '" DDS_SEC_PROP_AUTH_IDENTITY_CERT "'") == 0);
...
CU_Test(ddssec_builtin_validate_local_identity, missing_ca_property)
{
  ...
  participant_qos.property.value._buffer[0].value = ddsrt_strdup(identity_certificate);
  participant_qos.property.value._buffer[1].name = ddsrt_strdup(
    DDS_SEC_PROP_PREFIX "auth.identit_ca");  // deliberately *not* DDS_SEC_PROP_AUTH_IDENTITY_CA
  participant_qos.property.value._buffer[1].value = ddsrt_strdup(identity_ca);
  participant_qos.property.value._buffer[2].name = ddsrt_strdup(DDS_SEC_PROP_AUTH_PRIV_KEY);
  participant_qos.property.value._buffer[2].value = ddsrt_strdup(private_key_1024);
  ...
  CU_ASSERT(
    strcmp(
      exception.message,
      "validate_local_identity: missing property '" DDS_SEC_PROP_AUTH_IDENTITY_CA "'") == 0);
...
CU_Test(ddssec_builtin_validate_local_identity, missing_private_key_property)
{
  ...
  CU_ASSERT(
    strcmp(
      exception.message,
      "validate_local_identity: missing property '" DDS_SEC_PROP_AUTH_PRIV_KEY "'") == 0);
...
CU_Test(ddssec_builtin_validate_local_identity, unsupported_certification_format)
{
  ...
  fill_participant_qos(
    &participant_qos, false, cert, false, identity_ca, false, private_key, NULL, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
    &candidate_participant_guid, &exception);
...
CU_Test(ddssec_builtin_validate_local_identity, encrypted_key)
{
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, false, identity_ca, false,
    private_key_w_password, private_key_password, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
    &candidate_participant_guid, &exception);
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, false, identity_ca, true,
    private_key_w_password_filename, private_key_password, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
    &candidate_participant_guid, &exception);
...
CU_Test(ddssec_builtin_validate_local_identity, encrypted_key_no_password)
{
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, false, identity_ca, false,
    private_key_w_password, NULL, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
    &candidate_participant_guid, &exception);
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, false, identity_ca, true,
    private_key_w_password_filename, NULL, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
    &candidate_participant_guid, &exception);
...
CU_Test(ddssec_builtin_validate_local_identity, encrypted_key_invalid_password)
{
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, false, identity_ca, false,
    private_key_w_password, "invalid", NULL, false, NULL);
  ...
  fill_participant_qos(
    &participant_qos, false, identity_certificate, false, identity_ca, true,
    private_key_w_password_filename, "invalid", NULL, false, NULL);
...
CU_Test(ddssec_builtin_validate_local_identity, happy_day_elliptic)
{
  ...
  fill_participant_qos(
    &participant_qos, false, ec_identity_certificate, false, ec_identity_ca, false, ec_private_key,
    NULL, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
  ...
  fill_participant_qos(
    &participant_qos, true, ec_identity_certificate_filename, true, ec_identity_ca_filename, true,
    ec_private_key_filename, NULL, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
    &candidate_participant_guid, &exception);
...
CU_Test(ddssec_builtin_validate_local_identity, encrypted_ec_key)
{
  ...
  fill_participant_qos(
    &participant_qos, false, ec_identity_certificate, false, ec_identity_ca, false,
    ec_private_key_w_password, ec_password, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
    &candidate_participant_guid, &exception);
  ...
  fill_participant_qos(
    &participant_qos, false, ec_identity_certificate, false, ec_identity_ca, true,
    ec_private_key_w_password_filename, ec_password, NULL, false, NULL);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
    &candidate_participant_guid, &exception);
...
CU_Test(ddssec_builtin_validate_local_identity, elliptic_unsupported_certificate)
{
  ...
  fill_participant_qos(
    &participant_qos, false, ec_identity_certificate_unsupported, false, ec_identity_ca, false,
    ec_private_key, NULL, NULL, false, NULL);
...
CU_Test(ddssec_builtin_validate_local_identity, elliptic_unsupported_private_key)
{
  ...
  fill_participant_qos(
    &participant_qos, false, ec_identity_certificate, false, ec_identity_ca, false,
    ec_private_key_unsupported, NULL, NULL, false, NULL);
...
CU_Test(ddssec_builtin_validate_local_identity, no_file)
{
  ...
  fill_participant_qos(
    &participant_qos, true, identity_certificate_filename, false, identity_ca, true,
    "invalid_filename", NULL, NULL, false, NULL);
  ...
  fill_participant_qos(
    &participant_qos, true, identity_certificate_filename, true, "invalid_filename", true,
    private_key_filename, NULL, NULL, false, NULL);
  ...
  fill_participant_qos(
    &participant_qos, true, "invalid_filename", true, identity_ca_filename, false, private_key,
    NULL, NULL, false, NULL);
...
CU_Test(ddssec_builtin_validate_local_identity, with_extended_certificate_check)
{
  ...
  fill_participant_qos(
    &participant_qos, true, identity_certificate_filename, true, identity_ca_filename, true,
    private_key_filename, NULL, "trusted_ca_dir_not_matching", false, NULL);
...
CU_Test(ddssec_builtin_validate_local_identity, crl)
{
  ...
  fill_participant_qos(
    &participant_qos, false, revoked_identity_certificate, false, identity_ca, false,
    revoked_private_key, NULL, NULL, false, crl);
  /* Now call the function. */
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &adjusted_participant_guid, domain_id, &participant_qos,
    &candidate_participant_guid, &exception);
  ...
  fill_participant_qos(
    &participant_qos, true, revoked_identity_certificate_filename, true, identity_ca_filename, true,
    revoked_private_key_filename, NULL, NULL, true, crl_filename);
...
CU_Test(ddssec_builtin_validate_local_identity, trusted_ca_dir_and_crl)
{
  ...
  fill_participant_qos(
    &participant_qos, true, revoked_identity_certificate_filename, true, identity_ca_filename, true,
    revoked_private_key_filename, NULL, "trusted_ca_dir", true, crl_filename);

...
static const char * SUBJECT_NAME_IDENTITY_CERT =
  "CN=CHAM-574 client,O=Some Company,ST=Some-State,C=NL";
static const char * SUBJECT_NAME_IDENTITY_CA =
  "CN=CHAM-574 authority,O=Some Company,ST=Some-State,C=NL";
...
typedef enum { HANDSHAKE_REQUEST, HANDSHAKE_REPLY, HANDSHAKE_FINAL } HandshakeStep_t;
...
static void octet_seq_init(struct octet_seq * seq, unsigned char * data, uint32_t size)
{
...
static void octet_seq_deinit(struct octet_seq * seq) { ddsrt_free(seq->data); }

static void serializer_participant_data(
  DDS_Security_ParticipantBuiltinTopicData * pdata, unsigned char ** buffer, size_t * size);

static void dds_security_property_init(
  DDS_Security_PropertySeq * seq, DDS_Security_unsigned_long size)
{
  seq->_length = size;
  seq->_maximum = size;
...
static void initialize_identity_token(
  DDS_Security_IdentityToken * token, const char * certAlgo, const char * caAlgo)
{
...
static void initialize_permissions_token(DDS_Security_PermissionsToken * token, const char * caAlgo)
{
...
static DDS_Security_BinaryProperty_t * find_binary_property(
  DDS_Security_DataHolder * token, const char * name)
{
  ...
  for (i = 0; i < token->binary_properties._length && !result; i++) {
    if (
      token->binary_properties._buffer[i].name &&
      (strcmp(token->binary_properties._buffer[i].name, name) == 0)) {
...
static void deinitialize_identity_token(DDS_Security_IdentityToken * token)
{
  DDS_Security_DataHolder_deinit(token);
}

static int validate_local_identity(
  const char * ca_certificate, const char * public_cert, const char * key,
  const char * trusted_ca_dir, const char * crl_data)
{
  int res = 0;
  ...
  DDS_Security_GuidPrefix_t prefix = {0xb0, 0xb1, 0xb2, 0xb3, 0xb4, 0xb5,
...
#ifdef WIN32
    snprintf(
      trusted_ca_dir_path, 1024, "%s\\validate_begin_handshake_reply\\etc\\%s",
      CONFIG_ENV_TESTS_DIR, trusted_ca_dir);
#else
    snprintf(
      trusted_ca_dir_path, 1024, "%s/validate_begin_handshake_reply/etc/%s", CONFIG_ENV_TESTS_DIR,
      trusted_ca_dir);
#endif
    valbuf = &participant_qos.property.value._buffer[offset++];
  ...
  result = auth->validate_local_identity(
    auth, &local_identity_handle, &local_participant_guid, domain_id, &participant_qos,
    &candidate_participant_guid, &exception);

  if (result != DDS_SECURITY_VALIDATION_OK) {
    res = -1;
    printf(
      "validate_local_identity_failed: %s\n",
  ...
  initialize_identity_token(
    &local_participant_data->identity_token, RSA_2048_ALGORITHM_NAME, RSA_2048_ALGORITHM_NAME);
  ...
  serialized_participant_data._length = serialized_participant_data._maximum =
...
static void release_local_identity(void)
{
  DDS_Security_SecurityException exception = {NULL, 0, 0};
  ...
  if (local_identity_handle != DDS_SECURITY_HANDLE_NIL) {
    ...
    if (!success) {
      printf(
        "return_identity_handle failed: %s\n",
        exception.message ? exception.message : "Error message missing");
    }
    reset_exception(&exception);
  }
...
static X509 * load_certificate(const char * data)
{
  X509 * cert = NULL;
  BIO * bio;
...
static int get_adjusted_participant_guid(
...
static int create_dh_key_modp_2048(EVP_PKEY ** pkey)
{
  int r = 0;
...
static int get_dh_public_key_modp_2048(EVP_PKEY * pkey, struct octet_seq * pubkey)
{
  int r = 0;
  DH * dhkey;
...
static int create_dh_key_ecdh(EVP_PKEY ** pkey)
{
  int r = 0;
  EVP_PKEY * params = NULL;
...
static int get_dh_public_key_ecdh(EVP_PKEY * pkey, struct octet_seq * pubkey)
{
  int r = 0;
  EC_KEY * eckey = NULL;
  ...
  if (!(eckey = EVP_PKEY_get1_EC_KEY(pkey))) {
    ...
    (sz = EC_POINT_point2oct(group, point, POINT_CONVERSION_COMPRESSED, NULL, 0, NULL)) != 0) {
    pubkey->data = ddsrt_malloc(sz);
    pubkey->length = (uint32_t)EC_POINT_point2oct(
      group, point, POINT_CONVERSION_COMPRESSED, pubkey->data, sz, NULL);
    if (pubkey->length == 0) {
      char * msg = get_openssl_error_message_for_test();
...
static int validate_remote_identities(const char * remote_id_certificate)
{
  int res = 0;
  DDS_Security_ValidationResult_t result;
  ...
  DDS_Security_SecurityException exception = {NULL, 0, 0};
  DDS_Security_GuidPrefix_t prefix1 = {0xa0, 0xa1, 0xa2, 0xa3, 0xa4, 0xa5,
                                       0xa6, 0xa7, 0xa8, 0xa9, 0xaa, 0xab};
  DDS_Security_GuidPrefix_t prefix2 = {0xc0, 0xc1, 0xc2, 0xc3, 0xc4, 0xc5,
  ...
  initialize_identity_token(
    &remote_identity_token, RSA_2048_ALGORITHM_NAME, RSA_2048_ALGORITHM_NAME);

  result = auth->validate_remote_identity(
    auth, &remote_identity_handle1, &g_local_auth_request_token, NULL, local_identity_handle,
    &remote_identity_token, &remote_participant_guid1, &exception);

  if (
    (result != DDS_SECURITY_VALIDATION_PENDING_HANDSHAKE_REQUEST) &&
    (result != DDS_SECURITY_VALIDATION_PENDING_HANDSHAKE_MESSAGE)) {
    printf(
      "validate_remote_identity_failed: %s\n",
      exception.message ? exception.message : "Error message missing");
  }
  ...
  result = auth->validate_remote_identity(
    auth, &remote_identity_handle2, &local_auth_request_token, &g_remote_auth_request_token,
    local_identity_handle, &remote_identity_token, &remote_participant_guid2, &exception);
  ...
  initialize_identity_token(
    &remote_participant_data1->identity_token, RSA_2048_ALGORITHM_NAME, RSA_2048_ALGORITHM_NAME);
  initialize_permissions_token(
    &remote_participant_data1->permissions_token, RSA_2048_ALGORITHM_NAME);
  ...
  initialize_identity_token(
    &remote_participant_data2->identity_token, RSA_2048_ALGORITHM_NAME, RSA_2048_ALGORITHM_NAME);
  initialize_permissions_token(
  ...
  initialize_identity_token(
    &remote_participant_data3->identity_token, RSA_2048_ALGORITHM_NAME, RSA_2048_ALGORITHM_NAME);
  initialize_permissions_token(
  ...
  challenge1_predefined_glb =
    find_binary_property(&g_remote_auth_request_token, DDS_AUTHTOKEN_PROP_FUTURE_CHALLENGE);
  challenge2_predefined_glb = challenge1_predefined_glb;

  return res;
}

static void release_remote_identities(void)
{
  DDS_Security_SecurityException exception = {NULL, 0, 0};
  DDS_Security_boolean success;
...
CU_Init(ddssec_builtin_process_handshake)
{
  ...
  plugins = load_plugins(
    NULL /* Access Control */, &auth /* Authentication */, NULL /* Cryptograpy    */,
...
static bool compare_octet_seq(
  const DDS_Security_OctetSeq * seq1, const DDS_Security_OctetSeq * seq2)
{
  int r;
  if (seq1 && seq2) {
...
static void set_binary_property_value(
  DDS_Security_BinaryProperty_t * bp, const char * name, const unsigned char * data,
  uint32_t length)
{
  assert(bp);
  assert(name);
...
static void set_binary_property_string(
  DDS_Security_BinaryProperty_t * bp, const char * name, const char * data)
{
  uint32_t length;
...
static void fill_handshake_message_token(
  DDS_Security_HandshakeMessageToken * token, DDS_Security_ParticipantBuiltinTopicData * pdata,
  const char * certificate, const char * dsign, const char * kagree,
  const struct octet_seq * diffie_hellman1, const unsigned char * challengeData,
  unsigned int challengeDataSize, const struct octet_seq * diffie_hellman2,
  const unsigned char * challengeData2, unsigned int challengeDataSize2,
  const DDS_Security_BinaryProperty_t * hash1_from_request,
  const DDS_Security_BinaryProperty_t * hash2_from_reply, HandshakeStep_t step)
{
  ...
  switch (step) {
    case HANDSHAKE_REQUEST:
      ...
      serializer_participant_data(
        pdata, &serialized_local_participant_data, &serialized_local_participant_data_size);
      ...
      set_binary_property_value(
        c_pdata, DDS_AUTHTOKEN_PROP_C_PDATA, serialized_local_participant_data,
      ...
      /* Calculate the hash_c1 */
      {
        ...
        set_binary_property_value(
          hash_c1, DDS_AUTHTOKEN_PROP_HASH_C1, hash1_sentrequest_arr,
          sizeof(hash1_sentrequest_arr));
      }

      /* Set the DH public key associated with the local participant in dh1 property */
      if (diffie_hellman1) {
        set_binary_property_value(
          dh1, DDS_AUTHTOKEN_PROP_DH1, diffie_hellman1->data, diffie_hellman1->length);
      } else {
      ...
      if (challengeData) {
        set_binary_property_value(
          challenge1, DDS_AUTHTOKEN_PROP_CHALLENGE1, challengeData, challengeDataSize);
      } else {
        set_binary_property_value(
          challenge1, DDS_AUTHTOKEN_PROP_CHALLENGE1 "x", challenge1_predefined_glb->value._buffer,
          challenge1_predefined_glb->value._length);
      }
    ...
    case HANDSHAKE_REPLY:
      ...
      set_binary_property_value(
        hash_c1, DDS_AUTHTOKEN_PROP_HASH_C1, hash1_from_request->value._buffer,
        hash1_from_request->value._length);
      ...
      /* Calculate the hash_c2 */
      {
        ...
        set_binary_property_value(
          hash_c2, DDS_AUTHTOKEN_PROP_HASH_C2, hash2_sentreply_arr, sizeof(hash2_sentreply_arr));
      }
      ...
      if (challengeData) {
        ...
        set_binary_property_value(
          challenge1, DDS_AUTHTOKEN_PROP_CHALLENGE1 "x", challenge2->value._buffer,
          challenge2->value._length);
      }

      /* Set the challenge in challenge2 property */
      if (challengeData2) {
        set_binary_property_value(
          challenge2, DDS_AUTHTOKEN_PROP_CHALLENGE2, challengeData2, challengeDataSize2);
      } else {
        set_binary_property_value(
          challenge2, DDS_AUTHTOKEN_PROP_CHALLENGE2 "x", challenge2->value._buffer,
          challenge2->value._length);
      }
      ...
      if (diffie_hellman2) {
        set_binary_property_value(
          dh2, DDS_AUTHTOKEN_PROP_DH2, diffie_hellman2->data, diffie_hellman2->length);
      } else {
      ...
      /* Calculate the signature */
      {
        ...
        if (
          create_signature_for_test(
            private_key_x509, binary_properties, HANDSHAKE_SIGNATURE_SIZE, &sign, &signlen,
            &exception) != DDS_SECURITY_VALIDATION_OK) {
          printf("Exception: %s\n", exception.message);
        } else {
          set_binary_property_value(
            signature, DDS_AUTHTOKEN_PROP_SIGNATURE, sign, (uint32_t)signlen);
          ddsrt_free(sign);
        }
    ...
    case HANDSHAKE_FINAL:
      ...
      set_binary_property_value(
        ...
        hash1_from_request->value._length);
      set_binary_property_value(
        hash_c2, DDS_AUTHTOKEN_PROP_HASH_C2, hash2_from_reply->value._buffer,
        hash2_from_reply->value._length);
...
static void fill_handshake_message_token_default(
  DDS_Security_HandshakeMessageToken * token, DDS_Security_ParticipantBuiltinTopicData * pdata,
  const unsigned char * challengeData, unsigned int challengeDataSize)
{
  fill_handshake_message_token(
    token, pdata, remote_identity_certificate, AUTH_DSIGN_ALGO_RSA_NAME, AUTH_KAGREE_ALGO_RSA_NAME,
    &dh_modp_pub_key, challengeData, challengeDataSize, NULL, NULL, 0, NULL, NULL,
    HANDSHAKE_REQUEST);
}

static void handshake_message_deinit(DDS_Security_HandshakeMessageToken * token)
{
  DDS_Security_DataHolder_deinit(token);
}

static bool validate_handshake_token(
  DDS_Security_HandshakeMessageToken * token, const DDS_Security_OctetSeq * challenge1,
  const DDS_Security_OctetSeq * challenge2, HandshakeStep_t token_type)
{
  ...
  if (!token->class_id || strcmp(token->class_id, class_id) != 0) {
    ...
    CU_FAIL("HandshakeMessageToken incorrect property '" DDS_AUTHTOKEN_PROP_CHALLENGE1
            "' not found");
    ...
    CU_FAIL("HandshakeMessageToken incorrect property '" DDS_AUTHTOKEN_PROP_CHALLENGE2
            "' not found");
...
CU_Test(ddssec_builtin_process_handshake, happy_day_after_request)
{
  ...
  result = auth->begin_handshake_request(
    auth, &handshake_handle, &handshake_token_out, local_identity_handle, remote_identity_handle2,
    &serialized_participant_data, &exception);

  if (result != DDS_SECURITY_VALIDATION_PENDING_HANDSHAKE_MESSAGE) {
    printf(
      "begin_handshake_request failed: %s\n",
      exception.message ? exception.message : "Error message missing");
  }
  ...
  fill_handshake_message_token(
    ...
    AUTH_DSIGN_ALGO_RSA_NAME, AUTH_KAGREE_ALGO_ECDH_NAME, &dh1_pub_key,
    challenge1_glb->value._buffer, challenge1_glb->value._length, &dh_ecdh_pub_key,
    challenge2_predefined_glb->value._buffer, challenge2_predefined_glb->value._length,
    hash1_sentrequest, NULL, HANDSHAKE_REPLY);

  reset_exception(&exception);

  result = auth->process_handshake(
    auth, &handshake_reply_token_out, &handshake_reply_token_in, handshake_handle, &exception);

  CU_ASSERT_FATAL(result == DDS_SECURITY_VALIDATION_OK_FINAL_MESSAGE);
  CU_ASSERT(handshake_handle != DDS_SECURITY_HANDLE_NIL);
  CU_ASSERT(validate_handshake_token(
    &handshake_reply_token_out, &challenge1_glb->value, &challenge2_predefined_glb->value,
    HANDSHAKE_FINAL));
...
CU_Test(ddssec_builtin_process_handshake, happy_day_after_reply)
{
  ...
  fill_handshake_message_token_default(
    &handshake_token_in, remote_participant_data1, challenge1_predefined_glb->value._buffer,
    challenge1_predefined_glb->value._length);

  result = auth->begin_handshake_reply(
    auth, &handshake_handle, &handshake_token_out, &handshake_token_in, remote_identity_handle2,
    local_identity_handle, &serialized_participant_data, &exception);

  if (result != DDS_SECURITY_VALIDATION_PENDING_HANDSHAKE_MESSAGE) {
    printf(
      "begin_handshake_reply failed: %s\n",
      exception.message ? exception.message : "Error message missing");
  }
  ...
  CU_ASSERT(handshake_handle != DDS_SECURITY_HANDLE_NIL);
  CU_ASSERT(validate_handshake_token(
    &handshake_token_out, &challenge1_predefined_glb->value, NULL, HANDSHAKE_REPLY));
  ...
  fill_handshake_message_token(
    &handshake_final_token_in, NULL, remote_identity_certificate, AUTH_DSIGN_ALGO_RSA_NAME,
    AUTH_KAGREE_ALGO_ECDH_NAME, &dh_modp_pub_key, challenge1_predefined_glb->value._buffer,
    challenge1_predefined_glb->value._length, &dh2_pub_key, challenge2_glb->value._buffer,
    challenge2_glb->value._length, hash1_sentrequest, hash2_sentreply, HANDSHAKE_FINAL);

  result = auth->process_handshake(
    auth, &handshake_final_token_out, &handshake_final_token_in, handshake_handle, &exception);
  ...
  if (!success) {
    printf(
      "return_handshake_handle failed: %s\n",
      exception.message ? exception.message : "Error message missing");
  }
  reset_exception(&exception);
...
CU_Test(ddssec_builtin_process_handshake, invalid_arguments)
{
  ...
  fill_handshake_message_token(
    &handshake_final_token_in, NULL, remote_identity_certificate, AUTH_DSIGN_ALGO_RSA_NAME,
    AUTH_KAGREE_ALGO_RSA_NAME, &dh_modp_pub_key, challenge1_predefined_glb->value._buffer,
    challenge1_predefined_glb->value._length, &dh2_pub_key, challenge2_glb->value._buffer,
    challenge2_glb->value._length, hash1_sentrequest, hash2_sentreply, HANDSHAKE_FINAL);

  result =
    auth->process_handshake(auth, NULL, &handshake_final_token_in, handshake_handle, &exception);
  ...
  result = auth->process_handshake(auth, &handshake_token_out, NULL, handshake_handle, &exception);
  ...
  result =
    auth->process_handshake(auth, &handshake_token_out, &handshake_final_token_in, 0, &exception);
...
CU_Test(ddssec_builtin_process_handshake, invalid_dsign_algo)
{
  ...
  fill_handshake_message_token(
    &handshake_reply_token_in, remote_participant_data2, remote_identity_certificate,
    "RSASSA-PSS-SHA128", AUTH_KAGREE_ALGO_RSA_NAME, &dh1_pub_key, challenge1_glb->value._buffer,
    challenge1_glb->value._length, &dh_modp_pub_key, challenge2_predefined_glb->value._buffer,
    challenge2_predefined_glb->value._length, hash1_sentrequest, NULL, HANDSHAKE_REPLY);
...
CU_Test(ddssec_builtin_process_handshake, invalid_kagree_algo)
{
  ...
  fill_handshake_message_token(
    &handshake_reply_token_in, remote_participant_data2, remote_identity_certificate,
    AUTH_DSIGN_ALGO_RSA_NAME, "DH+MODP-2048-128", &dh1_pub_key, challenge1_glb->value._buffer,
    challenge1_glb->value._length, &dh_modp_pub_key, challenge2_predefined_glb->value._buffer,
    challenge2_predefined_glb->value._length, hash1_sentrequest, NULL, HANDSHAKE_REPLY);
...
CU_Test(ddssec_builtin_process_handshake, invalid_diffie_hellman)
{
  ...
  fill_handshake_message_token(
    &handshake_reply_token_in, remote_participant_data2, remote_identity_certificate,
    AUTH_DSIGN_ALGO_RSA_NAME, AUTH_KAGREE_ALGO_RSA_NAME, &invalid_dh_pub_key,
    challenge1_glb->value._buffer, challenge1_glb->value._length, &dh_modp_pub_key,
    challenge2_predefined_glb->value._buffer, challenge2_predefined_glb->value._length,
    hash1_sentrequest, NULL, HANDSHAKE_REPLY);
...
CU_Test(ddssec_builtin_process_handshake, return_handle)
{
  ...
  result = auth->begin_handshake_reply(
    auth, &handshake_handle, &handshake_token_out, &handshake_token_in, remote_identity_handle1,
    local_identity_handle, &serialized_participant_data, &exception);
...
CU_Test(ddssec_builtin_process_handshake, extended_certificate_check)
{
  ...
  CU_ASSERT_FATAL(!validate_local_identity(
    identity_ca, identity_certificate, private_key, "trusted_ca_dir", NULL));
  ...
  CU_ASSERT_FATAL(result == DDS_SECURITY_VALIDATION_PENDING_HANDSHAKE_MESSAGE);
  assert(
    result == DDS_SECURITY_VALIDATION_PENDING_HANDSHAKE_MESSAGE);  // for Clang's static analyzer
...
CU_Test(ddssec_builtin_process_handshake, crl)
{
  ...
  CU_ASSERT_FATAL(!validate_local_identity(
    revoker_identity_ca, local_identity_certificate, local_private_key, NULL, crl));

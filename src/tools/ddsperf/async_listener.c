...
enum async_listener_kind { ALK_DATA_AVAILABLE, ALK_PUBLICATION_MATCHED, ALK_SUBSCRIPTION_MATCHED };
...
struct async_listener * async_listener_new(void)
{
  ...
  if ((al = malloc(sizeof(*al))) == NULL) return NULL;
...
void async_listener_enqueue_data_available(
  struct async_listener * al, dds_on_data_available_fn fn, dds_entity_t rd, void * arg)
{
  async_listener_enqueue(
    al, (struct async_listener_event){
          .kind = ALK_DATA_AVAILABLE, .handle = rd, .fn_arg = arg, .u = {.da = {.fn = fn}}});
}

void async_listener_enqueue_subscription_matched(
  struct async_listener * al, dds_on_subscription_matched_fn fn, dds_entity_t rd,
  const dds_subscription_matched_status_t status, void * arg)
{
  async_listener_enqueue(
    al, (struct async_listener_event){
          ...
          .handle = rd,
          .fn_arg = arg,
          .u = {.sm = {.fn = fn, .st = status}}});
}

void async_listener_enqueue_publication_matched(
  struct async_listener * al, dds_on_publication_matched_fn fn, dds_entity_t wr,
  const dds_publication_matched_status_t status, void * arg)
{
  async_listener_enqueue(
    al, (struct async_listener_event){

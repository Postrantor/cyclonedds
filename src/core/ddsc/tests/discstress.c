...
static uint32_t createwriter_publisher(void * varg)
{
  ...
  topic =
    dds_create_topic(participant, &DiscStress_CreateWriter_Msg_desc, arg->topicname, qos, NULL);
  ...
  while (round < N_ROUNDS) {
    if (matched)
      ...
      if (matched) {
        printf(
          "All readers found; continuing at [%" PRIu32 ",%" PRIu32 "] for %d rounds\n",
...
static uint32_t wrinfo_hash(const void * va)
{
  ...
  return (
    uint32_t)(((a->rdid + UINT64_C(16292676669999574021)) * (a->wrid + UINT64_C(10242350189706880077))) >> 32);
}
...
static void dumplog(struct logbuf * logbuf)
{
  if (logbuf->line[logbuf->logidx][0])
    for (int i = logbuf->logidx; i < LOGDEPTH; i++) fputs(logbuf->line[i], stdout);
  for (int i = 0; i < logbuf->logidx; i++) fputs(logbuf->line[i], stdout);
  for (int i = 0; i < LOGDEPTH; i++) logbuf->line[i][0] = 0;
  logbuf->logidx = 0;
  fflush(stdout);
}

static bool checksample(
  struct ddsrt_hh * wrinfo, const dds_sample_info_t * si, const DiscStress_CreateWriter_Msg * s,
  struct logbuf * logbuf, uint32_t rdid)
{
  ...
  snprintf(
    logbuf->line[logbuf->logidx], sizeof(logbuf->line[logbuf->logidx]),
    "%" PRIu32 ": %" PRId32 ".%" PRIu32 " %" PRIu32 ".%" PRIu32 " iid %" PRIx64 " new %" PRIx64
    " st %c%c seq %" PRIu32 " seen %" PRIu32 "\n",
    rdid, s->round, s->wrseq, s->wridx, s->histidx, wri->wr_iid, si->publication_handle,
    (si->instance_state == DDS_IST_ALIVE)                ? 'A'
    : (si->instance_state == DDS_IST_NOT_ALIVE_DISPOSED) ? 'D'
                                                         : 'U',
    si->valid_data ? 'v' : 'i', s->seq, wri->seen);
  if (++logbuf->logidx == LOGDEPTH) logbuf->logidx = 0;

  if (wri->wr_iid != 0 && wri->wr_iid != si->publication_handle) {
    printf(
      "Mismatch between wrid %" PRIx64 " and publication handle %" PRIx64 "\n", wri->wr_iid,
      si->publication_handle);
    result = false;
  }
  if (wri->seen & (1u << s->histidx)) {
    printf(
      "Duplicate sample (wri->seen %" PRIx32 " s->histidx %" PRIu32 ")\n", wri->seen, s->histidx);
    result = false;
  }
  //if (s->histidx > 0)
  ...
  if (s->histidx > 0 && !(wri->seen & (1u << (s->histidx - 1)))) {
    printf(
      "Out of order sample (1) (wri->seen %" PRIx32 " s->histidx %" PRIu32 ")\n", wri->seen,
      s->histidx);
    result = false;
  }
  if (!(wri->seen < (1u << s->histidx))) {
    printf(
      "Out of order sample (2) (wri->seen %" PRIx32 " s->histidx %" PRIu32 ")\n", wri->seen,
      s->histidx);
    result = false;
  }
  wri->wr_iid = si->publication_handle;
  wri->seen |= 1u << s->histidx;
  if (!result) dumplog(logbuf);
  return result;
}
...
#define READ_LEN 3
      ...
      while ((n = dds_take(readers[xs[i]], raw, si, READ_LEN, READ_LEN)) > 0) {
        ...
        for (int32_t j = 0; j < n; j++) {
          ...
          if (si[j].valid_data && s->round >= 0) {
            if (!checksample(wrinfo, &si[j], s, &logbuf[xs[i]], (uint32_t)xs[i])) error = true;
          }
        }
        if (error) {
          fflush(stdout);
  ...
  for (struct wrinfo * wri = ddsrt_hh_iter_first(wrinfo, &it); wri; wri = ddsrt_hh_iter_next(&it)) {
    ...
    if (wri->seen != (1u << DEPTH) - 1) {
      printf(
        "err: wri->seen = %x rdid %" PRIu32 " wrid %" PRIu32 " iid %" PRIx64 " lna %d\n", wri->seen,
        wri->rdid, wri->wrid, wri->wr_iid, wri->last_not_alive);
      err++;
    }
